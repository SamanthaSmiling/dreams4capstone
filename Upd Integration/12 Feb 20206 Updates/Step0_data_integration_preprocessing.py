...
### START OF REPLACEMENT USA ###
                ...
                def _map_usa_scale(v):
                    if pd.isna(v) or v is None:
                        return None
                    s = str(v).strip()
                    return GAD_PHQ_MAP.get(s, None)
                ...
                if 'GAD_Bothered_1' in df.columns:
                    ...
                    unified_df['gad_total'] = df[gad_cols].apply(
                        lambda row: sum(
                            _map_usa_scale(v)
                            for v in row
                            if _map_usa_scale(v) is not None
                        ),
                        axis=1
                    )
                else:
...
### END OF REPLACEMENT USA ###
        ...
        for excel_file in data_form_files:
            ...
            try:
                ...
                field_mappings = {
                    ...
                    'dream_intensity': ['intensas.*emociones', r'dreamintensity(_\d+)?']
                }
                ...
                for unified_name, patterns in field_mappings.items():
                    ...
                    for col in df.columns:
                        ...
                        if any(re.search(pattern, col_str, flags=re.IGNORECASE) for pattern in patterns):
 ...
 ### START OF REPLACEMENT ARGENTINA ###               
                # --- Argentina GAD/PHQ totals: compute from item columns (robust to headers) ---

                # Helper: map typical Argentina response formats to 0-3 if needed
                ARG_SCALE = {
                    "nunca": 0, "para nada": 0, "0": 0, 0: 0,
                    "varios dias": 1, "varios días": 1, "1": 1, 1: 1,
                    "mas de la mitad de los dias": 2, "más de la mitad de los días": 2, "2": 2, 2: 2,
                    "casi todos los dias": 3, "casi todos los días": 3, "3": 3, 3: 3,
                }
                ...
                def to_0_3(x):
                    ...
                    if s.isdigit():
                        return float(int(s))
                    return float(ARG_SCALE.get(s, np.nan))

                # Find GAD item cols (prefer code headers like GAD_1 ... GAD_7)
                gad_item_cols = [c for c in df.columns if re.fullmatch(r"(?i)gad_\d+", str(c).strip())]
                # If not found, fallback: detect by Spanish question stem (sentirte nervioso/ansioso etc.)
                if not gad_item_cols:
                    gad_item_cols = [c for c in df.columns if "durante las últimas 2 semanas" in str(c).lower() and "preocup" in str(c).lower()]

                # Find PHQ item cols (PHQ9_1 ... PHQ9_9)
                phq_item_cols = [c for c in df.columns if re.fullmatch(r"(?i)phq9?_\d+", str(c).strip())]
                if not phq_item_cols:
                    phq_item_cols = [c for c in df.columns if "durante las últimas 2 semanas" in str(c).lower() and "molest" in str(c).lower()]

                # Compute totals
                if gad_item_cols:
                    gad_vals = df[gad_item_cols].applymap(to_0_3)
                    unified_df["gad_total"] = gad_vals.sum(axis=1, min_count=1).values
                else:
                    unified_df["gad_total"] = None

                if phq_item_cols:
                    phq_vals = df[phq_item_cols].applymap(to_0_3)
                    unified_df["phq_total"] = phq_vals.sum(axis=1, min_count=1).values
                else:
                    unified_df["phq_total"] = None
...
# OLD
                ...
                # for col in df.columns:
                #     s = str(col).lower()
                #     if gad_score_col is None and ('gad' in s) and ('score' in s) and ('final' not in s):
                #         gad_score_col = col
                #     if phq_score_col is None and ('phq' in s) and ('score' in s) and ('final' not in s):
                #         phq_score_col = col
...
### END OF REPLACEMENT ARGENTINA ###                
    ...
    def integrate_all_data(self):
        ...
        # Propagate demographics from timepoint 1 to all other timepoints for Argentina
        if not argentina_data.empty:
            argentina_data = self._propagate_argentina_demographics(argentina_data)
    ...
    def _propagate_argentina_demographics(self, df):
        """Propagate demographics from timepoint 1 to all other timepoints for Argentina participants"""
        df = df.copy()
        argentina_mask = df['country'] == 'Argentina'
        if not argentina_mask.any():
            return df
        
        demo_fields = ['age', 'education', 'student', 'gender']
        
        # Extract demographics from timepoint 1 for each participant
        argentina_data = df[argentina_mask].copy()
        timepoint_1_mask = (argentina_data['timepoint'] == 1) | (argentina_data['timepoint'] == 1.0)
        timepoint_1_data = argentina_data[timepoint_1_mask].copy()
        
        # Create a dictionary mapping participant_id to their timepoint 1 demographics
        # Use the first non-null value found for each demographic field
        demographics_dict = {}
        for idx, row in timepoint_1_data.iterrows():
            pid = str(row['participant_id']).strip() if pd.notna(row['participant_id']) else None
            if pid and pid != 'nan' and pid != 'None':
                if pid not in demographics_dict:
                    demographics_dict[pid] = {}
                for demo_field in demo_fields:
                    if demo_field in row.index:
                        value = row[demo_field]
                        # Only update if we don't have a value yet or if current value is null
                        if demo_field not in demographics_dict[pid] or pd.isna(demographics_dict[pid][demo_field]):
                            if pd.notna(value) and value is not None and str(value).strip() != '':
                                demographics_dict[pid][demo_field] = value
        
        # Fill in demographics for all timepoints
        for idx in df[argentina_mask].index:
            row = df.loc[idx]
            pid = str(row['participant_id']).strip() if pd.notna(row['participant_id']) else None
            if pid and pid != 'nan' and pid != 'None' and pid in demographics_dict:
                for demo_field in demo_fields:
                    if demo_field in df.columns:
                        # Only fill if the current value is missing
                        current_value = df.loc[idx, demo_field]
                        if pd.isna(current_value) or current_value is None or str(current_value).strip() == '':
                            value = demographics_dict[pid].get(demo_field, None)
                            if value is not None and pd.notna(value):
                                df.loc[idx, demo_field] = value
